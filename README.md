...
### ✅ هوشمندسازی evaluatePromptQuality — قدم ۱ از ۳ زیرقدم: تشخیص کلمات مبهم و تنوع واژگانی (MATTR) (ADR-118)

`scoreTextRichness` در `domain/workflow/WorkflowModels.kt` تا این قدم
فقط طول خام متن را می‌سنجید. اکنون دو سیگنال مستقل و سبک (نه AI،
On-Device) اضافه شد — **منبع محتوایی هر دو، تحقیق مستقل معمار در
منابع صنعت prompt engineering و یک مقاله‌ی تجربی روی ۱۰,۰۰۰ پرامپت
واقعی است، نه حدس**: (۱) تشخیص کلمات مبهم بدون جزئیات بصری (فهرست
دوزبانه انگلیسی/فارسی، تطبیق کلمه‌به‌کلمه‌ی واقعی نه substring —
`"goodness"` هرگز حاوی `"good"` شمرده نمی‌شود)؛ (۲) MATTR
(Moving-Average Type-Token Ratio، پنجره‌ی ۱۰ کلمه‌ای)، طبق یافته‌ی
تحقیقی معمار قوی‌ترین پیش‌بینی‌کننده‌ی منفرد کیفیت پرامپت. اگر بیش از
نیمی از کلمات متن مبهم باشند یا MATTR زیر ۰.۵ باشد، امتیاز یک «پله» در
فهرست ثابت `[0, 5, 12, 20]` پایین می‌آید (هر دو شرط همزمان = دو پله).
امضای بیرونی `evaluatePromptQuality`/`QualityScore` کاملاً بدون تغییر
ماند. ۳ تست تازه، هرکدام یک سیگنال را ایزوله می‌کند (کلمات مبهم
انگلیسی خالص، تکرار بدون کلمه‌ی مبهم با MATTR پایین، ترکیب دوزبانه)؛
...
### ✅ اتصال Rule های یتیم اعتبارسنجی پرامپت عکس مرجع (ADR-136)

زیرقدم ۲ فیچر «پرامپت ساخت عکس مرجع» (ADR-132) چهار تابع اعتبارسنجی
Warning-only نوشته بود که زیرقدم‌های ۴/۵ به UI وصل نکرده بودند — مانع
اصلی فنی بود: این توابع به خودِ رشته‌ی پرامپت *تولیدشده* نیاز دارند، نه
فیلدهای زنده‌ی فرم، پس با الگوی `validationIssues` موجود (که از فیلدهای
فرم محاسبه می‌شود) جور نبودند. این قدم یک StateFlow جدا
(`imagePromptValidationIssues`، و برای Outfit یک نسخه‌ی `Map` بر اساس
`outfit.id`) به هر سه ViewModel اضافه کرد که بلافاصله بعد از هر تولید
موفق (سریع یا AI) پر می‌شود و کنار نتیجه‌ی پرامپت نمایش داده می‌شود —
...
- ~~`validateCharacterUpdate`/`validateLocationUpdate`/`validatePropUpdate`
  (Hard/Soft Lock تداوم واحد ۰۶) هرگز از هیچ Screen/ViewModel واقعی صدا
  زده نمی‌شدند~~ — **رفع شد** (یافته‌ی حیاتی چکاپ جامع نهایی، ADR-143/144):
  تا این قدم می‌شد ظاهر/نام یک کاراکتر MAIN (سطح FULL) را آزادانه تغییر
  داد و ذخیره کرد، برخلاف تضمین صریح «FULL همیشه Blocking». اکنون هر سه
  فرم Asset (Character/Location/Object) پیش از `save()` واقعی، فیلد
  تغییریافته را با نسخه‌ی بارگذاری‌شده مقایسه می‌کنند: کاراکتر FULL با
  تغییر نام/ظاهر/سن Blocked می‌شود (ذخیره متوقف می‌شود)؛ کاراکتر MEDIUM،
  Location، و Object فقط Warning واقعی نشان می‌دهند (هرگز مانع ذخیره
  نمی‌شوند). **پیامد جدید:** چون مسیر رسمی «Character Evolution» هرگز
  پیاده نشده بود و در همین چکاپ به‌عنوان کد کاملاً مرده حذف شد
  (`EvolutionEntry`/`EvolutionTimeline`، ADR-142)، از این پس کاربری با
