...
- **واحد ۰۶:** `PhysicalAppearance`/`Gender` هنوز به شکل بلوپرینت v4/v5 (رشته‌ی آزاد `gender: String` به‌جای enum، nullable شدن `height`/`build`/`hair`/`facialFeatures`، افزودن `toPromptString()`) Migrate نشده‌اند — عمداً خارج از Scope دو قدم Migration اخیر (جزئیات در `docs/adr/029-unit06-continuity-tiers-migration-part1.md`).
- **واحد ۰۶:** `docs/reference/type-registry.md` هنوز فیلد `continuityLockLevel` را روی ردیف `CharacterAsset` فهرست نکرده — این فیلد در این Migration اضافه شد اما سند مرجع هنوز به‌روزرسانی نشده (جزئیات و دلیل در همان ADR).
