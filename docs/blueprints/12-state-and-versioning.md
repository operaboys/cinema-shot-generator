...
**نسخه:** ۲ — بازنویسی برای رفع یک تایپ گمشده کشف‌شده در بازبینی معماری مستقل: `Entity` در توابع `setLock`/`unlockWithOverride`/`analyzeVersionImpact` استفاده می‌شد اما هیچ‌جا تعریف نشده بود. اکنون به‌عنوان یک `interface` عمومی تعریف شده که هر Entity واقعی پروژه (Project/Scene/Shot/Asset) باید پیاده‌سازی کند. **تغییرات با «🆕v2» علامت‌گذاری شده‌اند.**
...
```kotlin
/**
 * 🆕v2 تعریف رسمی — تا این نسخه در متن نثر ارجاع داده می‌شد اما هیچ‌جا تعریف نشده بود.
 * یک قرارداد عمومی که هر Entity واقعی پروژه (Project/Scene/Shot/Asset، هرکدام در
 * بلوپرینت خودشان تعریف شده‌اند) باید آن را پیاده کند تا بتواند State/Lock/Version داشته باشد.
 * پیاده‌سازی واقعی معمولاً یک Wrapper سبک دور data class اصلی است (نه جایگزین آن)،
 * چون خودِ Scene/Shot/Asset نباید مستقیماً به این واحد وابسته شوند (طبق module-boundaries.md).
 *
 * ⚠️ نکته‌ی فنی: چون Entity یک interface است (نه data class)، متد copy() ندارد.
 * withLock() به‌جای entity.copy(lock=...) استفاده می‌شود — هر پیاده‌سازی واقعی این متد را
 * override می‌کند تا یک نسخه‌ی جدید از خودش با Lock جدید برگرداند.
 */
interface Entity {
    val id: String
    val type: String       // "project" | "scene" | "shot" | "asset" | ...
    val state: EntityState
    val lock: EntityLock
    fun withLock(newLock: EntityLock): Entity   // 🆕v2 جایگزین copy() که روی interface ممکن نیست
}
...
fun setLock(entity: Entity, locked: Boolean, reason: String? = null): Entity {
    return entity.withLock(EntityLock(locked, if (locked) nowIso8601() else null, reason))
}
...
fun unlockWithOverride(entity: Entity): Entity {
    ...
    return entity.withLock(EntityLock(locked = false))
}
```
...
🆕v2 با grep بررسی کن که `domain/stateversioning/` (کد واقعی، از قبل پیاده‌سازی و تست شده طبق ADR-014) از قبل `interface Entity` را دارد یا `setLock`/`unlockWithOverride` را با نوع دیگری (مثلاً یک `data class` مشخص به‌جای `interface` عمومی) پیاده کرده است:
- اگر پیاده‌سازی موجود از یک `data class` مشخص (نه `interface` عمومی) استفاده می‌کند، این بلوپرینت را با آن هماهنگ کن (کد تست‌شده اولویت دارد)، نه برعکس؛ در ADR مستند کن که چرا `interface` عمومی این‌جا لازم/غیرلازم بود.
- اگر پیاده‌سازی موجود اصلاً `Entity` نداشت (چون بلوپرینت تا این نسخه این تایپ را تعریف نکرده بود)، این تعریف را اضافه کن؛ توجه کن که `entity.copy(...)` روی یک `interface` ممکن نیست — از الگوی `withLock()` (طبق نمونه‌کد بالا) یا معادل مناسب دیگری استفاده کن.
- `Project`/`Scene`/`Shot`/`Asset` (تعریف‌شده در بلوپرینت‌های ۰۲/۰۴/۰۵/۰۶) خودشان نباید مستقیماً `Entity` را پیاده کنند (طبق `module-boundaries.md`، دامنه‌ی داده نباید به دامنه‌ی State/Versioning وابسته شود) — پیشنهاد: یک Wrapper سبک (مثلاً `data class VersionedEntityWrapper(val id, val type, val state, val lock, val underlying: Any)`) در لایه‌ی `data/repository/` این وابستگی را مدیریت کند؛ اگر معماری بهتری به ذهنت رسید، با دلیل در ADR پیشنهاد بده.
