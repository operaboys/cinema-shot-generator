...
| ۱۴ | **Output Delivery** ⭐ | `14-output-delivery.md` | Model Profile Library + Renderer + دوزبانگی خروجی پرامپت + دوزبانگی سراسری کل UI اپ |
...
بالای ۱۶ واحد در همین پوشه (`docs/blueprints/`) به‌عنوان بلوپرینت مستقل نوشته شده‌اند. **واحد ۱۷ به‌طور عمدی خارج از این پوشه است:**

| # | واحد | فایل | چرا مسیرش فرق دارد |
|---|---|---|---|
| ۱۷ | Universal Technical Variables | `docs/reference/universal-technical-variables.md` | این یک **کتابخانه‌ی مرجع** (فهرست سبک‌ها، نورها، زوایای دوربین) است، نه یک واحد با منطق کد و State مستقل — به همین دلیل در `docs/reference/` نگهداری می‌شود، مثل `glossary.md` و `naming-conventions.md`. از نظر معماری هنوز جزو ۱۸ واحد اصلی محسوب می‌شود و بقیه‌ی واحدها (Visual Identity، Camera & Motion، Scene Conditions) مستقیماً از آن استفاده می‌کنند. |
...
5. **اگر واحد شامل هر نوع UI (صفحه، فرم، پیام خطا) است**، کلیدهای ترجمه‌ی متناظر باید هم‌زمان در `fa.json` و `en.json` (طبق سیستم دوزبانگی سراسری در `14-output-delivery.md`، بخش ج) اضافه شوند — هیچ رشته‌ی متنی نباید مستقیم در کد Hardcode شود.
