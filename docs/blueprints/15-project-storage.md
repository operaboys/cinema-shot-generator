...
**نسخه:** ۲ — بازنویسی برای رفع یک تایپ گمشده کشف‌شده در بازبینی معماری مستقل: `ProjectData` در `validateReferentialIntegrity` استفاده می‌شد اما هیچ‌جا تعریف نشده بود. اکنون به‌عنوان یک `data class` Wrapper با `scenes`, `assets` (ترکیب هر سه نوع Character/Location/Object)، و `shots` تعریف شده است. **تغییرات با «🆕v2» علامت‌گذاری شده‌اند.**
...
```kotlin
/**
 * 🆕v2 تعریف رسمی — تا این نسخه به‌عنوان پارامتر validateReferentialIntegrity استفاده
 * می‌شد اما هیچ‌جا تعریف نشده بود. یک Wrapper سبک و موقت برای بررسی یکپارچگی، نه
 * ساختار ذخیره‌سازی واقعی (که در Room Entity ها، پایین‌تر در همین بلوپرینت، تعریف شده).
 *
 * ⚠️ نکته‌ی فنی: هر سه نوع Asset (CharacterAsset/LocationAsset/ObjectAsset، بلوپرینت ۰۶)
 * فیلد assetId مشترک دارند اما یک interface/کلاس پایه‌ی مشترک ندارند (طبق طراحی فعلی ۰۶).
 * برای این‌که validateReferentialIntegrity بتواند assetId را بدون بازتعریف نوع Asset ها
 * بخواند، این‌جا فقط شناسه‌ها (نه خودِ Asset کامل) نگه داشته می‌شوند.
 */
data class ProjectData(
    val projectId: String,
    val scenes: List<Scene>,
    val assetIds: Set<String>,   // شناسه‌ی تمام Asset ها (هر سه نوع)، نه خودِ Asset کامل
    val shots: List<Shot>
)
 ...
 * 🆕v2 project.assetIds اکنون مستقیماً Set<String> است (نه لیست Asset که map شود).
 */
fun validateReferentialIntegrity(project: ProjectData): List<IntegrityIssue> {
    ...
    project.shots.forEach { shot ->
        ...
        (shot.characterIds + shot.objectIds + shot.locationIds)
            .filterNot { it in project.assetIds }
            .forEach { missingId ->
...
🆕v2 با grep بررسی کن که `domain/storage/` یا `data/repository/` (کد واقعی، طبق ADR-017/018/019/020 از قبل پیاده‌سازی و تست شده) از قبل `data class ProjectData` را دارد یا `validateReferentialIntegrity` را با پارامترهای دیگری (مثلاً مستقیم چند `List` جدا به‌جای یک Wrapper) پیاده کرده است:
- اگر پیاده‌سازی موجود پارامترهای جدا دارد (که با توجه به ADR-019/020 محتمل است، چون آن‌ها به Repository های مجزا اشاره می‌کردند)، همان امضای موجود را در این بلوپرینت منعکس کن (کد تست‌شده اولویت دارد)، نه برعکس؛ در ADR مستند کن.
- اگر `ProjectData` اصلاً وجود نداشت، این تعریف را طبق بالا اضافه کن — توجه به این‌که `assetIds: Set<String>` (نه لیست Asset کامل) به‌عمد انتخاب شده تا از وابستگی غیرضروری به سه نوع Asset مختلف در یک تابع ساده جلوگیری شود.
