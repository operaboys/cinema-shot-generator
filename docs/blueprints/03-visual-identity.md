...
**نسخه:** ۲ — بازنویسی برای هم‌راستاسازی با Migration بزرگ بلوپرینت `02-dna-manager-v2.md` (نسخه ۳): `getPacingFromEmotion` دیگر رشته‌ی خام نمی‌گیرد، بلکه enum کامل `Mood` (تعریف‌شده در بلوپرینت ۰۲) را می‌پذیرد. **تغییرات با «🆕v2» علامت‌گذاری شده‌اند.**
...
/** 🆕v2 نگاشت احساس اصلی Scene (از DNA.globalMoodBase.primaryEmotion) به یک پیش‌فرض ریتم.
...
fun getPacingFromEmotion(emotion: Mood): CinematicMode = when (emotion.category) {
    MoodCategory.DARK, MoodCategory.HIGH_ENERGY -> CinematicMode.FAST_CUT
    MoodCategory.CALM, MoodCategory.EMOTIONAL -> CinematicMode.LONG_TAKE
    else -> CinematicMode.BALANCED
}
```
...
این Migration باید **بعد از** Migration بلوپرینت ۰۲ (نسخه ۳) اجرا شود، چون به `Mood`/`MoodCategory` از آن‌جا وابسته است:

- `getPacingFromEmotion` باید `import` واقعی `Mood`/`MoodCategory` از `domain.dna` داشته باشد.
- ⚠️ **تغییر رفتاری واقعی، نه فقط تغییر نوع:** نگاشت جدید بر اساس `MoodCategory` (۵ دسته) است، نه فهرست دستی رشته‌ها (که فقط ۶ مقدار خاص را می‌شناخت و بقیه را نادیده می‌گرفت با `else -> BALANCED`). این یعنی مثلاً `Mood.EPIC` (که در `MoodCategory.HIGH_ENERGY` است) اکنون `FAST_CUT` می‌گیرد، در حالی که نسخه‌ی قبلی هیچ نگاشتی برایش نداشت و به `BALANCED` می‌افتاد. این تغییر را با تست‌های موجود `PacingTest`/مشابه (اگر وجود دارد) به‌طور صریح مقایسه کن و در گزارش/ADR بگو کدام Mood ها رفتار متفاوتی نسبت به قبل گرفتند.
- اگر تست‌های موجود این تابع روی ورودی‌های رشته‌ای (`"tense"`, ...) نوشته شده‌اند، باید به `Mood.TENSE` و مشابه به‌روزرسانی شوند، نه صرفاً حذف شوند.
