...
```
docs/blueprints/[NN]-[unit-name].md    (فایل مستقیم، نه پوشه — ساختار واقعی رپو)
docs/governance/[topic-name].md
docs/reference/[topic-name].md
```

نسخه‌های بازنویسی‌شده‌ی یک بلوپرینت، پسوند `-v2`/`-v3`/... به همان نام فایل اضافه می‌کنند (مثلاً `02-dna-manager-v2.md`)، نه یک فایل کاملاً جدید یا یک پوشه‌ی نسخه‌بندی‌شده.
