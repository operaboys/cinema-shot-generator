...
class SchemaAnalyzer:
    ...
    def parse_sql_ddl(self, ddl_content: str) -> None:
        ...
        # Find each CREATE TABLE statement, then extract its body by counting
        # parentheses (not a non-greedy regex) so nested parens in column
        # definitions (VARCHAR(100), DECIMAL(10,2), REFERENCES table(col))
        # don't truncate the match at the first ")" encountered.
        header_pattern = re.compile(r'CREATE\s+TABLE\s+(\w+)\s*\(', re.IGNORECASE)

        for header_match in header_pattern.finditer(ddl_content):
            table_name = header_match.group(1).lower()
            body_start = header_match.end()  # just after the opening "("
            depth = 1
            i = body_start
            while i < len(ddl_content) and depth > 0:
                if ddl_content[i] == '(':
                    depth += 1
                elif ddl_content[i] == ')':
                    depth -= 1
                i += 1
            if depth != 0:
                continue  # unbalanced parens for this statement; skip rather than mis-parse
            table_definition = ddl_content[body_start:i - 1]
            ...
            if part.upper().startswith('PRIMARY KEY'):
                ...
                if column:
                    ...
                    if column.foreign_key:
                        # Inline "column TYPE REFERENCES table(col)" syntax —
                        # collect these too, not just explicit FOREIGN KEY(...) clauses,
                        # so table-level foreign_keys reflects all FKs, not just some.
