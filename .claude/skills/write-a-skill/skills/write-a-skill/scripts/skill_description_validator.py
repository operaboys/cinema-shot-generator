    ...
    if args.path:
        try:
            ...
            if os.path.isdir(resolved_path):
                # Match the other two scripts' UX: accept a skill folder and
                # auto-locate SKILL.md inside it, instead of erroring with
                # "Is a directory".
                candidate = os.path.join(resolved_path, "SKILL.md")
                if not os.path.isfile(candidate):
                    print(f"error: no SKILL.md found in directory {resolved_path}", file=sys.stderr)
                    return 1
