...
1. **Scans** your codebase for every WCAG 2.2 Level A and AA violation, categorized by severity (Critical, Serious, Moderate)
...
| **Severity Classification** | Categorizes each violation as Critical, Serious, or Moderate |
...
| **Serious** | Significant barrier that degrades experience | Insufficient color contrast, missing form labels | Fix within current sprint |
| **Moderate** | Usability issue that causes friction | Redundant ARIA roles, suboptimal heading hierarchy | Fix within next 2 sprints |
...
# Check color contrast for specific values (colors are positional, not --fg/--bg)
python scripts/contrast_checker.py "#777777" "#ffffff"

# Check contrast across a CSS file (extracts color pairs automatically)
python scripts/contrast_checker.py --batch /path/to/styles.css

# Suggest an accessible alternative for a given foreground color
...
Options:
  ...
  --severity LIST          Comma-separated severity filter, e.g. critical,serious (values: critical, serious, moderate)
...
Positional:
  foreground               Foreground (text) color: #RRGGBB, #RGB, rgb(r,g,b), or named color
  background                Background color: same formats

Options:
  --suggest COLOR          Suggest accessible alternative colors for the given foreground
  --batch CSS_FILE         Extract color pairs from a CSS file and check each
  --json                   Output results as JSON
  --demo / --sample        Run with embedded sample color pairs
```

Note: there is no `--tailwind` or `--level` flag — the tool reports all AA/AAA levels (normal text, large text, UI components) in one pass; filter the output for the level you need.
