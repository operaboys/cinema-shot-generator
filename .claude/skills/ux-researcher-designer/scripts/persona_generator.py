...
class PersonaGenerator:
    ...
    def _generate_tagline(self, patterns: Dict, archetype: str = None) -> str:
        """Generate persona tagline.

        Derives wording from the archetype (already classified by
        _identify_archetype from these same patterns) rather than
        independently recomputing the dominant frequency/context via
        max() — that independent computation could disagree with the
        archetype (e.g. 'daily' being the most common frequency, but not
        enough distinct features used to qualify as power_user, so the
        archetype falls through to 'casual_user') and produce a tagline
        that contradicts the persona's own label, e.g. "Archetype: Casual
        User" paired with "A daily user... work purposes".
        """
        archetype_taglines = {
            'power_user': "A power user who relies on the product daily to get work done efficiently",
            'mobile_first': "A mobile-first user who engages with the product primarily on the go",
            'business_user': "A business user who primarily uses the product for work purposes",
            'casual_user': "A casual user who uses the product occasionally for their needs",
        }
        if archetype and archetype in archetype_taglines:
            return archetype_taglines[archetype]

        # Fallback (no archetype supplied): previous behavior.
