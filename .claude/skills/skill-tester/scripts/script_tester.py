...
class TestSuite:
    ...
    def calculate_summary(self):
        ...
        # PASS requires every script to have actually passed — zero explicit
        # failures is NOT sufficient, because a script can be entirely
        # "PARTIAL" (some sub-tests failed) without any script being marked
        # "FAIL" outright. Reporting that case as PASS would contradict this
        # skill's own rule: "never report a partial pass" (see Verification
        # Loop in SKILL.md).
        if self.summary["no_tests"] > 0:
            self.summary["overall_status"] = "NO_TESTS"
        elif self.summary["failed"] > 0:
            self.summary["overall_status"] = "FAIL"
        elif self.summary["partial"] > 0:
            self.summary["overall_status"] = "PARTIAL"
        elif self.summary["passed"] == self.summary["total_scripts"]:
