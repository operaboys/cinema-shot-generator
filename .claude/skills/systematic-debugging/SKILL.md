...
description: Reproduce-then-isolate debugging discipline. Use this skill for ANY bug, test failure, crash, wrong output, flaky behavior, or "it worked yesterday" situation — BEFORE proposing any fix. It stops guess-driven fixes by forcing you to read the full error, reproduce it, name one hypothesis, and change one thing at a time.
---
...
- `bug-fix-scope` — this skill finds the root cause; that one makes sure the fix is applied
  project-wide, not just at the reported spot.
- `verification-before-completion` — the "test that now proves it" must actually run, fresh,
