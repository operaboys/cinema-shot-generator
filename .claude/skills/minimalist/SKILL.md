...
Minimalism cuts speculative code, not risk controls. Never simplify away, in the name of "the user didn't ask for this":

- Input validation at trust boundaries (anything from a user, network, or file)
- Error handling for operations that can genuinely fail (I/O, network calls, parsing external data) — this is different from handling errors "that can't happen"
- Security checks (auth, authorization, sanitization)
- Data-loss-preventing checks (confirmations before destructive operations)
- Accessibility in UI code

If a request would need one of these cut to hit "minimum code," say so explicitly and keep the check — don't silently drop it to look leaner.
