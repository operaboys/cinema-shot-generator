...
class ComponentScanner:
    ...
    # Named function declarations, e.g. "export function Button(...) {".
    # This is distinct from FUNCTIONAL_COMPONENT above, which only matches
    # arrow functions (const X = (...) =>) or anonymous function expressions
    # (function (...)) — it never matched a named function declaration like
    # "function Button(...)", even though that's one of the two most common
    # React component styles (and the exact style in this skill's own
    # documented example, "export function Button({ className, children })").
    NAMED_FUNCTION_COMPONENT = re.compile(
    ...
    def _scan_file(self, file_path: Path):
        ...
        # Find named function declaration components (e.g. "function Button(...)"),
        # which FUNCTIONAL_COMPONENT above does not match. Skip names already
        # found to avoid double-counting.
        for match in self.NAMED_FUNCTION_COMPONENT.finditer(content):
            name = match.group(1)
