...
def analyze_plan(text, source):
    ...
    if not steps:
        return {
            ...
            "percentage": 0.0,
            "step_results": [],
            "recommendations": ["No plan steps detected — write the plan as numbered or bulleted steps so each can be checked for a verification criterion."],
        }
    ...
    for step in steps:
        ...
        step_results.append({
            ...
            "score": pts,
            "level": level,
            "has_verify_label": bool(VERIFY_LABEL.search(step["title"] + "\n" + step["body"])),
        })
