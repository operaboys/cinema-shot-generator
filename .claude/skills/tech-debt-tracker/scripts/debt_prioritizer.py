...
class DebtPrioritizer:
    ...
    def _estimate_effort(self, item: Dict[str, Any]) -> EffortEstimate:
        ...
        if debt_type in ["architecture_debt", "security_risk", "large_file",
                          "hardcoded_credentials", "sql_injection_risk"]:
            # hardcoded_credentials and sql_injection_risk are the actual
            # security-related debt_type values debt_scanner.py emits;
            # "security_risk" alone never matches anything the scanner
            # produces, so real security findings were silently scored as
            # low-risk (1.0) here despite being flagged "critical" severity.
            risk_factor = 1.8
