...
class DebtScanner:
    ...
    def _compile_patterns(self):
        ...
        self.smell_patterns = {
            ...
            # No pattern here previously caught hardcoded credentials at all —
            # the skill's own sample_codebase ships a flagship example of
            # exactly this (self.stripe_key = "sk_test_...") specifically to
            # demonstrate detection, yet it was invisible to every prior
            # pattern (hardcoded_paths only matches path-like "/a/b" strings).
            # Word-boundary note: uses (?:^|[_\s]) instead of \b before the
            # keyword so prefixed names like STRIPE_KEY / API_SECRET still
            # match — \b does not match between a letter and "_".
            "hardcoded_credentials": re.compile(
                r"(?i)[\w.]*(?:api[_-]?key|key|secret|token|password|passwd)\s*"
                r"[:=]\s*['\"][A-Za-z0-9_\-/+=]{8,}['\"]"
            ),
        }
    ...
    def _scan_common_patterns(self, file_path: str, content: str, lines: List[str]):
        ...
        # Code smells
        # hardcoded_credentials is a security finding, not a style nit like
        # print_debug or hardcoded_paths — it gets its own (higher) severity
        # instead of the blanket "medium" every other smell type receives.
        smell_severity = defaultdict(lambda: "medium")
        smell_severity["hardcoded_credentials"] = "critical"
