...
**CSV format** — columns: `assumption, category, risk, certainty`. `risk` and `certainty` must each be a decimal in the range **0 to 1** (not a 1-5 scale) — e.g. `risk=0.9` for very high risk, `certainty=0.3` for low certainty. A value outside 0-1 raises a `ValueError` on load.
...
CLI utility that:
- reads assumptions from CSV or inline input (columns: assumption, category, risk, certainty — risk and certainty as decimals in [0, 1])
