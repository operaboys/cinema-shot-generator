...
class DesignTokenGenerator:
    ...
    def _export_as_scss(self, tokens: Dict) -> str:
        ...
        Was previously called by export_tokens() but didn't exist anywhere
        in this class, so every `--format scss` request crashed with
        AttributeError. Modeled on _export_as_css above (same recursive
        flatten of the nested token dict), using SCSS variable syntax
        ($token-name: value;) instead of CSS custom properties, and no
        surrounding :root block since SCSS variables aren't scoped that way.
        """
        ...
        def flatten_dict(obj, prefix=''):
            for key, value in obj.items():
                if isinstance(value, dict):
                    ...
                    scss.append(f'${prefix}-{key}: {value};')
